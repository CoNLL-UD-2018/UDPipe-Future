...
for lang in ${@:-$(sort ../langs)}; do
  for result in */$lang-*.eval; do
    echo -n "${result%.eval} "; (cat $result; echo) | head -n1
  done | sort -srnk2
  echo
done | perl -pe 's/(\d+\.\d+)/sprintf "%.2f", $1/ge' | column -te | less -S
#lang="$1"; shift
#for cmd in "$@"; do
#  ./udpipe --tokenizer=normalized_spaces tokenizer-ori/$lang.model ../ud-2.2-conll18/$lang/$lang-ud-dev.txt \
