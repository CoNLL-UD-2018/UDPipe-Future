    ...
    def construct(self, args, num_words, num_chars, num_tags, num_deprels, predict_only):
        with self.session.graph.as_default():
            ...
            for tag in args.tags:
                ...
                if tag == "LEMMAS" and args.lemma_rnn:
                    if args.lemma_ce_shared:
                        lemma_ce_embeddings = character_embeddings
                    else:
                        lemma_ce_embeddings = tf.get_variable("lemma_ce_embeddings", shape=[num_chars, args.cle_dim], dtype=tf.float32)
                    lemma_form_embedded = tf.nn.embedding_lookup(lemma_ce_embeddings, self.charseqs)
                    lemma_form_encoded = tf.layers.dropout(lemma_form_embedded, rate=args.dropout, training=self.is_training)
                    for i in range(args.lemma_rnn):
                        (output_fwd, output_bwd), (state_fwd, state_bwd) = tf.nn.bidirectional_dynamic_rnn(
                            tf.nn.rnn_cell.GRUCell(args.cle_dim), tf.nn.rnn_cell.GRUCell(args.cle_dim),
                            lemma_form_encoded, sequence_length=self.charseq_lens, dtype=tf.float32,
                            scope="lemma-rnn-{}".format(i))
                        if i < args.lemma_rnn - 1:
                            lemma_form_encoded = output_fwd + output_bwd + lemma_form_encoded
                        else:
                            lemma_form_encoded = tf.concat([state_fwd, state_bwd], axis=1)
                    tag_layer = tf.concat([tag_layer, tf.nn.embedding_lookup(lemma_form_encoded, self.charseq_ids[:, 1:])], axis=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lemma_ce_shared", default=0, type=int, help="Do share lemma rnn ce embeddings.")
    parser.add_argument("--lemma_rnn", default=0, type=int, help="Lemma rnn layers.")
